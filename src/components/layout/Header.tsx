...
function getLocalizedPath(currentPath: string, targetLocale: 'az' | 'en' | 'tr' | 'ru' | 'fr' | 'it' | 'ar' | 'zh' | 'hi' | 'es' | 'pt' | 'nl' | 'de'): string {
  ...
  const paths: Record<string, Record<'az' | 'en' | 'tr' | 'ru' | 'fr' | 'it' | 'ar' | 'zh' | 'hi' | 'es' | 'pt' | 'nl' | 'de', string>> = {
    '/': { az: '/', en: '/', tr: '/', ru: '/', fr: '/', it: '/', ar: '/', zh: '/', hi: '/', es: '/', pt: '/', nl: '/', de: '/' },
    '/reseptler': { az: '/reseptler', en: '/recipes', tr: '/recipes', ru: '/recipes', fr: '/recipes', it: '/recipes', ar: '/recipes', zh: '/recipes', hi: '/recipes', es: '/recipes', pt: '/recipes', nl: '/recipes', de: '/recipes' },
    '/recipes': { az: '/reseptler', en: '/recipes', tr: '/recipes', ru: '/recipes', fr: '/recipes', it: '/recipes', ar: '/recipes', zh: '/recipes', hi: '/recipes', es: '/recipes', pt: '/recipes', nl: '/recipes', de: '/recipes' },
    '/haqqinda': { az: '/haqqinda', en: '/about', tr: '/about', ru: '/about', fr: '/about', it: '/about', ar: '/about', zh: '/about', hi: '/about', es: '/about', pt: '/about', nl: '/about', de: '/about' },
    '/about': { az: '/haqqinda', en: '/about', tr: '/about', ru: '/about', fr: '/about', it: '/about', ar: '/about', zh: '/about', hi: '/about', es: '/about', pt: '/about', nl: '/about', de: '/about' },
    '/xidmetler': { az: '/xidmetler', en: '/services', tr: '/services', ru: '/services', fr: '/services', it: '/services', ar: '/services', zh: '/services', hi: '/services', es: '/services', pt: '/services', nl: '/services', de: '/services' },
    '/services': { az: '/xidmetler', en: '/services', tr: '/services', ru: '/services', fr: '/services', it: '/services', ar: '/services', zh: '/services', hi: '/services', es: '/services', pt: '/services', nl: '/services', de: '/services' },
    '/elaqe': { az: '/elaqe', en: '/contact', tr: '/contact', ru: '/contact', fr: '/contact', it: '/contact', ar: '/contact', zh: '/contact', hi: '/contact', es: '/contact', pt: '/contact', nl: '/contact', de: '/contact' },
    '/contact': { az: '/elaqe', en: '/contact', tr: '/contact', ru: '/contact', fr: '/contact', it: '/contact', ar: '/contact', zh: '/contact', hi: '/contact', es: '/contact', pt: '/contact', nl: '/contact', de: '/contact' },
    '/privacy': { az: '/privacy', en: '/privacy', tr: '/privacy', ru: '/privacy', fr: '/privacy', it: '/privacy', ar: '/privacy', zh: '/privacy', hi: '/privacy', es: '/privacy', pt: '/privacy', nl: '/privacy', de: '/privacy' },
    '/terms': { az: '/terms', en: '/terms', tr: '/terms', ru: '/terms', fr: '/terms', it: '/terms', ar: '/terms', zh: '/terms', hi: '/terms', es: '/terms', pt: '/terms', nl: '/terms', de: '/terms' },
    '/resept': { az: `/resept/${slug}`, en: `/recipe/${slug}`, tr: `/recipe/${slug}`, ru: `/recipe/${slug}`, fr: `/recipe/${slug}`, it: `/recipe/${slug}`, ar: `/recipe/${slug}`, zh: `/recipe/${slug}`, hi: `/recipe/${slug}`, es: `/recipe/${slug}`, pt: `/recipe/${slug}`, nl: `/recipe/${slug}`, de: `/recipe/${slug}` }
  };
      ...
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-white/60 bg-[rgba(255,251,246,0.82)] shadow-[0_18px_60px_rgba(52,34,22,0.12)] backdrop-blur-xl">
        <div className="hidden items-center justify-between border-b border-[rgba(98,67,45,0.08)] px-6 py-3 text-xs font-medium uppercase tracking-[0.22em] text-[rgba(95,59,37,0.72)] md:flex">
          <div className="flex items-center gap-3">
            ...
            <span>{locale === 'en' ? 'Daily 08:00 - 22:00' : locale === 'tr' ? 'Her Gün 08:00 - 22:00' : locale === 'ru' ? 'Ежедневно 08:00 - 22:00' : locale === 'fr' ? 'Tous les jours 08:00 - 22:00' : locale === 'it' ? 'Tutti i giorni 08:00 - 22:00' : locale === 'ar' ? 'يوميًا 08:00 - 22:00' : locale === 'zh' ? '每日 08:00 - 22:00' : locale === 'hi' ? 'रोजाना 08:00 - 22:00' : locale === 'es' ? 'Todos los días 08:00 - 22:00' : locale === 'pt' ? 'Todos os dias 08:00 - 22:00' : locale === 'nl' ? 'Dagelijks 08:00 - 22:00' : locale === 'de' ? 'Täglich 08:00 - 22:00' : 'Hər gün 08:00 - 22:00'}</span>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="flex items-center justify-between px-5 py-4 sm:px-6">
          <Link href={locale === 'en' ? "/en" : locale === 'tr' ? "/tr" : locale === 'ru' ? "/ru" : locale === 'fr' ? "/fr" : locale === 'it' ? "/it" : locale === 'ar' ? "/ar" : locale === 'zh' ? "/zh" : locale === 'hi' ? "/hi" : locale === 'es' ? "/es" : locale === 'pt' ? "/pt" : locale === 'nl' ? "/nl" : locale === 'de' ? "/de" : "/"} className="flex items-center gap-2.5 group">
          ...
          <nav className="hidden items-center gap-1 lg:flex">
            {navigation.map((item) => {
              const isActive = pathname === item.href || (item.href !== '/' && item.href !== '/en' && item.href !== '/tr' && item.href !== '/ru' && item.href !== '/fr' && item.href !== '/it' && item.href !== '/ar' && item.href !== '/zh' && item.href !== '/hi' && item.href !== '/es' && item.href !== '/pt' && item.href !== '/nl' && item.href !== '/de' && pathname?.startsWith(item.href));
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <Button asChild variant="outline" className="rounded-full border-[rgba(98,67,45,0.14)] bg-white/70 px-5 text-[rgba(57,44,35,0.82)] hover:bg-white">
              <Link href={locale === 'en' ? "/en/recipes" : locale === 'tr' ? "/tr/recipes" : locale === 'ru' ? "/ru/recipes" : locale === 'fr' ? "/fr/recipes" : locale === 'it' ? "/it/recipes" : locale === 'ar' ? "/ar/recipes" : locale === 'zh' ? "/zh/recipes" : locale === 'hi' ? "/hi/recipes" : locale === 'es' ? "/es/recipes" : locale === 'pt' ? "/pt/recipes" : locale === 'nl' ? "/nl/recipes" : locale === 'de' ? "/de/recipes" : "/reseptler"}>
            ...
            <div className="flex items-center gap-1 rounded-full border border-[rgba(98,67,45,0.1)] bg-white/72 p-0.5">
              {(['az', 'en', 'tr', 'ru', 'fr', 'it', 'ar', 'zh', 'hi', 'es', 'pt', 'nl', 'de'] as const).map((lang) => {
              ...
              <div className="space-y-2 px-4 py-4">
                ...
                <div className="grid gap-2 pt-2">
                  <Button asChild className="rounded-full bg-[rgba(141,58,36,0.96)] text-white">
                    <Link href={locale === 'en' ? "/en/recipes" : locale === 'tr' ? "/tr/recipes" : locale === 'ru' ? "/ru/recipes" : locale === 'fr' ? "/fr/recipes" : locale === 'it' ? "/it/recipes" : locale === 'ar' ? "/ar/recipes" : locale === 'zh' ? "/zh/recipes" : locale === 'hi' ? "/hi/recipes" : locale === 'es' ? "/es/recipes" : locale === 'pt' ? "/pt/recipes" : locale === 'nl' ? "/nl/recipes" : locale === 'de' ? "/de/recipes" : "/reseptler"} onClick={() => setIsMenuOpen(false)}>{t.header.viewRecipes}</Link>
                  </Button>
