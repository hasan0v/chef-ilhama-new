      ...
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[1.25rem] sm:rounded-[2.2rem] border border-[rgba(98,67,45,0.1)] bg-[linear-gradient(135deg,rgba(34,27,23,0.98),rgba(59,40,28,0.96))] text-white shadow-[0_28px_90px_rgba(26,18,12,0.34)]">
        <div className="grid gap-10 px-6 py-10 sm:px-8 lg:grid-cols-[1.15fr_0.85fr_0.8fr_1fr] lg:px-12 lg:py-14">
          ...
          <div className="space-y-4">
            ...
            <ul className="space-y-2">
              <li><Link href={locale === 'en' ? "/en/recipes" : locale === 'tr' ? "/tr/recipes" : locale === 'ru' ? "/ru/recipes" : locale === 'fr' ? "/fr/recipes" : locale === 'it' ? "/it/recipes" : locale === 'ar' ? "/ar/recipes" : locale === 'zh' ? "/zh/recipes" : locale === 'hi' ? "/hi/recipes" : locale === 'es' ? "/es/recipes" : locale === 'pt' ? "/pt/recipes" : locale === 'nl' ? "/nl/recipes" : locale === 'de' ? "/de/recipes" : "/reseptler"} className="text-sm text-white/68 transition-colors hover:text-white sm:text-base">{t.nav.recipes}</Link></li>
              <li><Link href={locale === 'en' ? "/en/about" : locale === 'tr' ? "/tr/about" : locale === 'ru' ? "/ru/about" : locale === 'fr' ? "/fr/about" : locale === 'it' ? "/it/about" : locale === 'ar' ? "/ar/about" : locale === 'zh' ? "/zh/about" : locale === 'hi' ? "/hi/about" : locale === 'es' ? "/es/about" : locale === 'pt' ? "/pt/about" : locale === 'nl' ? "/nl/about" : locale === 'de' ? "/de/about" : "/haqqinda"} className="text-sm text-white/68 transition-colors hover:text-white sm:text-base">{t.nav.about}</Link></li>
              <li><Link href={locale === 'en' ? "/en/services" : locale === 'tr' ? "/tr/services" : locale === 'ru' ? "/ru/services" : locale === 'fr' ? "/fr/services" : locale === 'it' ? "/it/services" : locale === 'ar' ? "/ar/services" : locale === 'zh' ? "/zh/services" : locale === 'hi' ? "/hi/services" : locale === 'es' ? "/es/services" : locale === 'pt' ? "/pt/services" : locale === 'nl' ? "/nl/services" : locale === 'de' ? "/de/services" : "/xidmetler"} className="text-sm text-white/68 transition-colors hover:text-white sm:text-base">{t.nav.services}</Link></li>
              <li><Link href={locale === 'en' ? "/en/contact" : locale === 'tr' ? "/tr/contact" : locale === 'ru' ? "/ru/contact" : locale === 'fr' ? "/fr/contact" : locale === 'it' ? "/it/contact" : locale === 'ar' ? "/ar/contact" : locale === 'zh' ? "/zh/contact" : locale === 'hi' ? "/hi/contact" : locale === 'es' ? "/es/contact" : locale === 'pt' ? "/pt/contact" : locale === 'nl' ? "/nl/contact" : locale === 'de' ? "/de/contact" : "/elaqe"} className="text-sm text-white/68 transition-colors hover:text-white sm:text-base">{t.nav.contact}</Link></li>
            </ul>
          </div>
