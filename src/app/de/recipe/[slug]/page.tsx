...
export async function generateStaticParams() {
  const recipes = await getRecipes('de');
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!recipe) {
    return {
      title: 'Rezept nicht gefunden - Chef İlhamə',
    };
  }

  return {
    title: `Rezept für ${recipe.name} - Chef İlhamə`,
    description: recipe.history ? recipe.history.substring(0, 160) : `Detaillierte Zubereitungsschritte und Zutatenliste für ${recipe.name}.`,
  };
}
